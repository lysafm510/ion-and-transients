...
def INITIAL_PARAMETER():
    ...
    for E in range(0, NE):  # NE为三角形单元的数量,计算D和V
        ...
        TOTAL_AREA = TOTAL_AREA + AREA  # 所有三角形面积和

        All_of_det[E] = DET  # 存储每个三角形的D
        All_of_area[E] = AREA  # 存储每个三角形的A

        for i in range(0, 3):
            CTRL_AREA[NOD[i, E]] = CTRL_AREA[NOD[i, E]] + AREA  # 每一点的控制面积 以该点为中心的周围三角形面积和

    for i in range(0, NP):  # ？点的控制面积相加不是会有重叠吗
    ...
    nmax = int(max(count))  # 一个点周围最大的三角形个数
    # print("NMAX ", nmax)
    # print("NP ", NP)
    # print("NE ", NE)
...
def CORRECTION_EQUATION(iteration, step):
    ...
    # print("Vj ---------- ", Vj)
    # print("DCARYR ------ ", DCARYR)
    RELEASE_STEP = int((RELEASE_TIMES + (10 ** -6)) / DT)  # 10000步关通道
