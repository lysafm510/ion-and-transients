...
data_frame = pd.read_csv("avg_gn_jsr_gauss_new_2.csv", header=None)
...
def cal_t50():
    ...
    peak_time_s = peak_time * 1000
    print("tRise：%s" % peak_time_s)
...
def cal_FDHM():
    ...
    half_time1 = index * 100 * dt - (right_concentration - half_concentration) * dt / (
            right_concentration - left_concentration)
    print("下降到一半时间：%s" % (half_time1 * 1000))
    ...
    if index >= len(data) - 1:
        ...
        half_time2 = index * 100 * dt - (right_concentration - half_concentration) * dt / (
                right_concentration - left_concentration)
        print("回升到一半时间：%s" % (half_time2 * 1000))
