...
def INITIAL_PARAMETER():
    ...
    # Jrefill,Jryr两项
    # 扩散系数
    DCARYR = 0.25 * 6.5 * 10 ** 7  # RyR通道 Ca离子扩散系数
...
def LOAD_GRIDINFO():
    ...
    count = 0
    gridt_file = open('../../parameters/grid/blink/' + GRID_PATH + '/gridt.dat', 'r')  # gridt.dat，这个是空间离散化的每一个离散的节点的坐标，第一行是节点的数量
    ...
    count = 0
    gridt_file = open('../../parameters/grid/blink/' + GRID_PATH + '/npoch.dat', 'r')  # npoch.dat为每个节点的属性：0代表内部节点，2代表入流边界或远场的节点，1代表壁面的节点，4代表出流边界节点
    ...
    count = 0
    gridt_file = open('../../parameters/grid/blink/' + GRID_PATH + '/nod.dat', 'r')  # nod.dat，这个是gridt.dat中这些节点组成的三角形单元的信息，第一行是三角形单元的数量，后面是每一个三角元对应的三个节点的编号；
    ...
    count = 0
    gridt_file = open('../../parameters/grid/blink/' + GRID_PATH + '/noe.dat',
