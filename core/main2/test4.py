...
def LOAD_GRIDINFO():
    ...
    count = 0
    gridt_file = open('../../parameters/grid/blink/' + GRID_PATH + '/gridt.dat',
                      'r')  # gridt.dat，这个是空间离散化的每一个离散的节点的坐标，第一行是节点的数量
    ...
    count = 0
    gridt_file = open('../../parameters/grid/blink/' + GRID_PATH + '/npoch.dat',
    ...
    count = 0
    gridt_file = open('../../parameters/grid/blink/' + GRID_PATH + '/nod.dat',
...
def get_d_ca_ryr(k_ryr):
    global vm
    C = 2.0  # 膜电容
    R = 8.314  # 理想气体常数
    T = 310  # 温度
    Z = 2  # 化合价
    F = UNITEC * MOLNUM * (10 ** 4)  # 法拉第常数

    i_ca_ryr = CA_CURRENT()
    vm = vm - i_ca_ryr * DT / (C * TOTAL_AREA) * (10.0 ** 11)
    print("vm", vm)
    c_ca_store = cca_ryr_inside_store()
